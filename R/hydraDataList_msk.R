#' Data for Hydra dat and pin files derived from mskeyrun (hydradata)
#'
#'All the required information for creating dat and pin files are stored in this list. This
#'data list is used to create documentation and to create dat and pin files for use in simulaion with Hydra
#'
#' @format A list containing 140 elements
#' \describe{
...
#'   \item{areaMortality}{Numeric Vector: mortality rate outside modeled area}
...
#'   \item{avgFPhase}{Integer scalar: estimation phase to for average fishing mortality F}
#'   \item{avgRecPhase}{Integer scalar: estimation phase to for average recruitment}
...
#'   \item{csigPhase}{Integer scalar: estimation phase for catch observation error c sigma}
#'   \item{debugState}{Integer scalar: Type of debugging. (Values 1, 2, 3, 4). Default = 3}
#'   \item{devFPhase}{Integer scalar: estimation phase for fishing mortality F deviations}
#'   \item{devRecPhase}{Integer scalar: estimation phase for recruitment deviations}
...
#'   \item{Fdevs}{Numeric matrix (Nfleets x Nyrs): estimation starting values for annual deviations from average fishing mortality F by fleet}
...
#'   \item{fisheryq}{Numeric vector (Nqpars): Fish caught per fish available per unit effort and per unit time. Worded in a different way it represents the probability of any single fish being caught}
#'   \item{fisherySigma}{Numeric matrix (Nspecies x Nfleets): Standard deviation applied to catch (assumed log normal distribution)}
#'   \item{fishsel_pars}{Numeric matrix (2 parameters c and d x Nfleets): parameters of logistic curve representing proportion caught at fish length}
...
#'   \item{fleetMembership}{Numeric vector for estimation model: which numbered fleet catches which species, identical to q indicator }
...
#'   \item{fqphase}{Integer scalar: estimation phase for fishery catchability, q}
#'   \item{fsphase}{Integer scalar: estimation phase for fishery selectivity parameters}
...
#'   \item{indicatorFisheryq}{Integer vector (Nspecies): Determines which species are actually targeted catch (1+ by fleet number) or not (0). This is used to determine exploitation rate durin an assessment}
...
#'   \item{intakeStomach}{Numeric matrix (Nspecies x Nsizebins): Each row represents species stomach weight (g) for each size class. Note. It is assumed that stomach weight remains constant through time (only for creating data input files. Hydra is modelled to acept changes through time.) }
...
#'   \item{residenceTime}{Numeric Vector: proportion of time spent in modeled area}
