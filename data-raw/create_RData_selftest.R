...
create_RData_selftest <- function(dattype = c("sim", "real"), 
                                  nlenbin = 5) {
  ...
  if(dattype == "real"){
    ...
    focalspp <- mskeyrun::focalSpecies %>%
      ...
      dplyr::filter(modelName %in% c("Atlantic_cod", 
                                     "Atlantic_herring", 
                                     "Atlantic_mackerel", 
                                     "Spiny_dogfish")) %>%
  ...
  hydraDataList_st <- d
  hydraDataList_st <- modifyList(hydraDataList_st,p)
  ...
  usethis::use_data(hydraDataList_st,overwrite = TRUE)
...
get_DatData_msk <- function(dattype,
  ...
  lenwt <- survbiopar %>%
    dplyr::filter(Name %in% focalspp$Name) %>%
    dplyr::mutate(species = Name) %>%
    dplyr::arrange(species)
...
get_PinData_msk <- function(dattype,
  ...
  redundantAvgRec <- redundantAvgRec %>%
    dplyr::select(unique(speciesList))
